...
QUOTED_DB_USERNAME="\"$POSTGRES_DB_USERNAME\""
QUOTED_DB_SCHEMA="\"$POSTGRES_DB_SCHEMA\""
...
echo "Creating roles..."
psql "user=$POSTGRES_ADMIN_USERNAME password=$POSTGRES_ADMIN_PASSWORD host=$POSTGRESQL_HOST port=$POSTGRESQL_PORT dbname=$POSTGRES_DATABASE" -c "CREATE ROLE $QUOTED_POSTGRES_DB_USERNAME WITH LOGIN    NOSUPERUSER    NOCREATEDB    NOCREATEROLE    NOINHERIT    NOREPLICATION    CONNECTION LIMIT -1 PASSWORD '$POSTGRES_DB_PASSWORD';"

echo "Creating schema..."
psql "user=$POSTGRES_ADMIN_USERNAME password=$POSTGRES_ADMIN_PASSWORD host=$POSTGRESQL_HOST port=$POSTGRESQL_PORT dbname=$POSTGRES_DATABASE" -c "CREATE SCHEMA IF NOT EXISTS $QUOTED_POSTGRES_DB_SCHEMA AUTHORIZATION $QUOTED_POSTGRES_DB_USERNAME;"
