...
import {
  InvoiceResponse,
  InvoicesResponse,
} from '../../dto/response/invoice/invoiceResponse';
...
export class InvoiceResolver {
  ...
  async getInvoices(
    ...
    try {
      ...
      if (invoices?.length > 0) {
        return this.invoicesRes.createResponse(
          'Invoice records fetched successfully',
          HttpStatus.OK,
          true,
          invoices,
        );
  ...
  async getInvoiceById(
    ...
    try {
      ...
      if (invoice) {
        return this.invoiceRes.createResponse(
          'Invoice record fetched successfully',
          HttpStatus.OK,
          true,
          invoice,
        );
  ...
  async createInvoice(
    ...
    try {
      this.loggerService.log(
        `InvoiceResolver: createInvoice: invoice: ${JSON.stringify(invoice)}`,
      );
  ...
  async updateInvoice(
    ...
    try {
      this.loggerService.log(
        `InvoiceResolver: updateInvoice: invoice: ${JSON.stringify(invoice)}`,
      );
