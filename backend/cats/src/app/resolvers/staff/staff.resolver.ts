...
} from '../../dto/response/staff/staffResponse';
...
export class StaffResolver {
  ...
  async findAll(
    ...
    @Args({ name: 'filter', type: () => Filter, nullable: true })
    ...
    try {
      ...
      this.loggerSerivce.log(
        `page: ${page}, pageSize: ${pageSize}, filter: ${filter}, sortBy: ${sortBy}, sortByDir: ${sortByDir}`,
      );
  ...
  async getApplicationsByStaff(
    ...
    @Args({ name: 'roleId', type: () => Int, nullable: true })
    roleId?: number | null,
  ) {
    try {
      this.loggerSerivce.log(
      ...
      if (result?.data?.length > 0) {
        ...
        this.loggerSerivce.log(
          'StaffResolver.getApplicationsByStaff() RES:404 end',
        );
        return this.applicationResponse.createPagedResponse({
