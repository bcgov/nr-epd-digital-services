...
export class InvoiceService {
      ...
      if (id <= 0) {
        ...
        const result = {
          ...
          recipient: {
            ...
            value: `${invoice?.recipient?.firstName || ''} ${
              invoice?.recipient?.middleName || ''
            } ${invoice?.recipient?.lastName || ''}`.trim(),
    ...
    try {
      ...
      const result = plainToInstance(ViewInvoice, {
        ...
        recipient: {
          ...
          value: `${createdInvoice?.recipient?.firstName || ''} ${
            createdInvoice?.recipient?.middleName || ''
          } ${createdInvoice?.recipient?.lastName || ''}`.trim(),
  ...
  async sendInvoice(invoice: InvoiceEmail, attachment?: any) {
    try {
      // Parse To recipients from JSON string
      const toRecipients: { email: string; personId?: number | null }[] =
        JSON.parse(invoice.to || '[]');
      const toEmails = toRecipients.map((r) => r.email).filter(Boolean);
      ...
      const ccRecipients: { email: string; personId?: number | null }[] =
        invoice.cc ? JSON.parse(invoice.cc) : [];
      const ccEmails = ccRecipients.map((r) => r.email).filter(Boolean);
      ...
      this.loggerService.error(
        `InvoiceService: sendInvoice: Error sending invoice: ${error.message}`,
        null,
      );
      throw new Error(
        `Failed to send invoice ID ${invoice?.invoiceId}: ${error.message}`,
      );
    }
  }
}
