...
export class PersonNoteService {
  ...
  async getPersonNotesByPersonId(personId: number, user: any) {
    ...
    try {
      ...
      this.loggerSerivce.debug(
        `Fetching notes for personId: ${personId} by user: ${user?.givenName}`,
      );

      if (user?.identity_provider === UserTypeEum.IDIR) {
        // Log the condition where the identity_provider matches IDIR
        this.loggerSerivce.debug(
          `Identity provider is IDIR for user: ${user?.givenName}`,
        );

        const notes = await this.personNoteRepository.findBy({
          personId,
        ...
        if (notes?.length > 0) {
          this.loggerSerivce.log(
            `Found ${notes.length} notes for personId: ${personId}`,
          );
        ...
        this.loggerSerivce.warn(
          `Invalid identity_provider for user: ${user?.givenName}. Expected IDIR but got ${user?.identity_provider}`,
        );
        return [];
      }
    } catch (err) {
      ...
      this.loggerSerivce.error(
        `Exception occurred in NoteService.getNotesByPersonId() for personId: ${personId}`,
        JSON.stringify(err),
      );
      throw new HttpException(
        `Failed to retrieve notes for person id: ${personId}`,
  ...
  async createPersonNote(createNote: CreatePersonNote, user: any) {
    ...
    try {
      ...
      this.loggerSerivce.debug(
        `Attempting to create note with description: ${createNote?.noteDescription} by user: ${user?.givenName}`,
      );
      ...
      if (user?.identity_provider === UserTypeEum.IDIR) {
        ...
        if (savedNote) {
          this.loggerSerivce.log(
            `Note created successfully with ID: ${savedNote.id}`,
          );
          return {
            id: savedNote.id,
          ...
          this.loggerSerivce.warn(
            'Note creation failed, no data returned from save operation',
          );
          return null;
        }
      } else {
        ...
        this.loggerSerivce.warn(
          `Note creation blocked: User ${user?.givenName} is not using IDIR identity provider.`,
        );
        throw new HttpException(
          'Only users with IDIR identity provider are allowed to create notes',
      ...
      this.loggerSerivce.error(
        'Exception occurred in NoteService.createNote()',
        JSON.stringify(err),
      );
  ...
  async updatePersonNote(
    noteId: string,
    updateNote: UpdatePersonNote,
    user: any,
  ) {
    // Early return if the user is not authorized to perform this action
    if (user?.identity_provider !== UserTypeEum.IDIR) {
      this.loggerSerivce.warn(
        `User ${user?.givenName} blocked from updating note. IDIR is required.`,
      );
      throw new HttpException(
        'Only users with IDIR identity provider are allowed to update notes',
        HttpStatus.FORBIDDEN,
      );
    }

    this.loggerSerivce.debug(
      `Attempting to update note with ID: ${noteId} by user: ${user?.givenName}`,
    );

    try {
      // Retrieve the note to be updated
      const note = await this.personNoteRepository.findOne({
        where: { id: noteId },
      });
      ...
      if (!note) {
        ...
        this.loggerSerivce.warn(
          `Note with ID: ${noteId} not found for user: ${user?.givenName}`,
        );
      ...
      if (isUpdated) {
        ...
        note.noteDescription =
          updateNote.noteDescription ?? note.noteDescription;
        ...
        // Log success
        this.loggerSerivce.log(
          `Note updated successfully with ID: ${updatedNote.id}`,
        );
        ...
        this.loggerSerivce.log(
          `No changes detected for note with ID: ${noteId}. No update required.`,
        );
      ...
      this.loggerSerivce.error(
        'Exception occurred in NoteService.updateNote()',
        JSON.stringify(err),
      );
  ...
  async deletePersonNote(notes: DeletePersonNote[], user: any) {
    ...
    if (user?.identity_provider !== UserTypeEum.IDIR) {
      this.loggerSerivce.warn(
        `User ${user?.givenName} blocked from deleting note. IDIR is required.`,
      );
      throw new HttpException(
        'Only users with IDIR identity provider are allowed to delete notes',
        HttpStatus.FORBIDDEN,
      );
    }

    this.loggerSerivce.debug(
      `Attempting to delete note(s) with ID(s): ${notes} by user: ${user?.givenName}`,
    );

    // Perform bulk delete operation regardless of the number of IDs in the array
    const results = await Promise.all(
      notes.map(async (personNote) => {
        try {
          ...
          if (!note) {
            ...
            this.loggerSerivce.warn(
              `Note with ID: ${personNote.id} not found for user: ${user?.givenName}`,
            );
            return false;
          }
          ...
          if (result) {
            ...
            this.loggerSerivce.log(
              `Note with ID: ${personNote.id} successfully deleted by user: ${user?.givenName}`,
            );
            return true;
          } else {
            // Log the unsuccessful deletion
            this.loggerSerivce.log(
              `Note with ID: ${personNote.id} is not successfully deleted by user: ${user?.givenName}`,
            );
            return false;
          }
        } catch (err) {
          // Handle and log any internal errors
          this.loggerSerivce.error(
            'Exception occurred in NoteService.deleteNote()',
            JSON.stringify(err),
          );
          throw new HttpException(
            `Failed to delete ${failedDeletes.length} note(s). See details for failed IDs.`,
