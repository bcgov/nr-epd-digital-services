...
describe('DashboardService', () => {
  ...
  let recentViewedApplicationRepository: Partial<
    Repository<RecentViewedApplication>
  >;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        DashboardService,
        {
          provide: getRepositoryToken(RecentViewedApplication),
          useValue: recentViewedApplicationRepository,
        },
  ...
  describe('getRecentViewedApplications', () => {
    ...
    it('should log and throw on repository error', async () => {
      ...
      (recentViewedApplicationRepository.find as jest.Mock).mockRejectedValue(
        error,
      );

      await expect(
        dashboardService.getRecentViewedApplications(user),
  ...
  describe('createRecentViewedApplication', () => {
    ...
    it('should create and save a recent viewed application successfully', async () => {
      ...
      (recentViewedApplicationRepository.remove as jest.Mock).mockResolvedValue(
        undefined,
      );
      (recentViewedApplicationRepository.save as jest.Mock).mockImplementation(
        (x) => Promise.resolve(x),
      );
    ...
    it('should remove oldest applications if more than max limit', async () => {
      ...
      await dashboardService.createRecentViewedApplication(
        application,
        userInfo,
      );

      expect(recentViewedApplicationRepository.remove).toHaveBeenCalledWith(
        oldApps.slice(0, 1)[0],
      );
    });
    ...
    it('should log and throw on repository or service errors', async () => {
      ...
      (recentViewedApplicationRepository.save as jest.Mock).mockRejectedValue(
        new Error('save fail'),
      );
  ...
  describe('getApplications', () => {
    it('should return a list of processed applications with site address', async () => {
      ...
      expect(loggerService.log).toHaveBeenCalledWith(
        'DashboardService.getApplications() start',
      );
      ...
      expect(loggerService.log).toHaveBeenCalledWith(
        'DashboardService.getApplications() end',
      );
    });
    ...
    it('should throw error if dataSource.query fails', async () => {
      ...
      await expect(dashboardService.getApplications()).rejects.toThrow(
        'Database error',
      );
      expect(loggerService.error).toHaveBeenCalledWith(
