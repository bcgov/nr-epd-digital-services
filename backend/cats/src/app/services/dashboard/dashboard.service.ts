...
export class DashboardService {
  ...
  async getRecentViewedApplications(user: any) {
    this.loggerService.log(
      'DashboardService.getRecentViewedApplication() start',
    );
    try {
      ...
      const recentViewedApplication =
        await this.recentViewedApplicationRepository.find({
          where: { userId: userId },
          order: { visitedDateTime: 'ASC' },
        });
      this.loggerService.log(
        'DashboardService.getRecentViewedApplication() end',
      );
  ...
  async getApplications() {
    try {
      ...
      this.loggerService.log(
        'Fetching top 5 applications based on priority and received date',
      );
      ...
      if (!applications || applications.length === 0) {
        ...
        const result = await Promise.all(
          applications.map(async (app: any) => {
            this.loggerService.log(
              `Processing application ID: ${app.applicationid}, siteId: ${app.siteid}`,
            );
            this.loggerService.log(
              `Fetching site details for application ID: ${app.applicationid}, siteId: ${app.siteid}`,
            );
            ...
            if (app?.siteid != null && app.siteid.toString().length > 0) {
              ...
              this.loggerService.log(
                `Got site details for application ID: ${app.applicationid}, siteId: ${app.siteid}`,
              );
              const siteDetails = site?.findSiteBySiteIdLoggedInUser?.data;
              if (siteDetails) {
                siteAddress = [
  ...
  async createRecentViewedApplication(application: Application, userInfo: any) {
    this.loggerService.log(
      'DashboardService.createRecentViewedApplication() start',
    );
    try {
      ...
      const recentlyVisitedApplications =
        await this.getRecentViewedApplications(userInfo);
      this.loggerService.log(
        `Found ${recentlyVisitedApplications.length} recently visited applications for user: ${userId}`,
      );

      //currently existed recently visited applications for user
      const existingApplications = recentlyVisitedApplications.find(
        (app) =>
          app.applicationId === application?.id &&
          app.userId === userId &&
          app.siteId === application?.siteId,
      );
      this.loggerService.log(
        `Found existing recently visited applications for user: ${userId} and application ID: ${application?.id} and site ID: ${application?.siteId}`,
      );

      if (existingApplications) {
        this.loggerService.log(
          `Updating existing recently visited application for user: ${userId}, application ID: ${application?.id} and site ID: ${application?.siteId}`,
        );
        existingApplications.visitedDateTime = new Date();
        const updatedRecentViewedApplication =
          await this.recentViewedApplicationRepository.save(
        ...
        if (recentlyVisitedApplications.length >= maxVisitedApplications) {
          ...
          this.loggerService.log(
            `Deleting oldest recently visited application for user: ${userId}, application ID: ${applicationsToDelete?.applicationId} and site ID: ${applicationsToDelete?.siteId}`,
          );
          await this.recentViewedApplicationRepository.remove(
        ...
        this.loggerService.log(
          `Creating recent viewed application for user: ${userId}, application ID: ${application?.id}`,
        );
        let siteAddress = '';
        const siteInfo = site?.findSiteBySiteIdLoggedInUser?.data;
        if (siteInfo) {
        ...
        recentViewedApplication.applicationType =
