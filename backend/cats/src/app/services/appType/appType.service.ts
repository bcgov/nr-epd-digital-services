...
export class AppTypeService {
  ...
  async getAllAppTypes() {
    this.loggerService.log('AppTypeService.getAllAppTypes() start');
    ...
    try {
      ...
      this.loggerService.log('AppTypeService.getAllAppTypes() end');
      return appTypes;
    } catch (err) {
      this.loggerService.error(
        'Exception occurred in AppTypeService.getAllAppTypes()',
        JSON.stringify(err),
      );
      throw new HttpException(
