...
const _checkPermission = async (
  { currentObject, currentUser, params },
  permission,
) => {
  ...
  const userId = await userService.getCurrentUserId(
    getCurrentIdentity(currentUser, SYSTEM_USER),
  );
  ...
  if (authType === AuthType.BEARER && userId) {
    ...
    if (params.objectId) {
      permissions.push(
        ...(await objectPermissionService.searchPermissions({
    ...
    if (params.bucketId || currentObject.bucketId) {
      permissions.push(
        ...(await bucketPermissionService.searchPermissions({
          bucketId: params.bucketId || currentObject.bucketId,
          ...searchParams,
        })),
      );
    }
...
const checkAppMode = (req, _res, next) => {
  ...
  try {
    ...
    throw new Problem(501, {
      detail:
        'Current application mode does not support incoming authentication type',
...
const hasPermission = (permission) => {
  return async (req, _res, next) => {
    ...
    const canOidcMode = (mode) =>
      [AuthMode.OIDCAUTH, AuthMode.FULLAUTH].includes(mode);

    try {
      if (!canOidcMode(authMode)) {
        log.debug(
          'Current application mode does not enforce permission checks',
          { function: 'hasPermission' },
        );
        ...
        log.debug('Basic authTypes are always permitted', {
          function: 'hasPermission',
        });
      } else if (
        req.params.objectId &&
        req.currentObject.public &&
        permission === Permissions.READ
      ) {
        log.debug('Read requests on public objects are always permitted', {
          function: 'hasPermission',
        });
