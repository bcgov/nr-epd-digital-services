...
exports.up = function (knex) {
  return (
    Promise.resolve()
      ...
      .then(() =>
        knex.schema.createTable('bucket', (table) => {
      ...
      .then(() =>
        knex.schema.createTable('bucket_permission', (table) => {
      ...
      .then(() =>
        knex.schema.alterTable('object', (table) => {
          table
            .uuid('bucketId')
            .references('bucketId')
            .inTable('bucket')
      ...
      .then(() =>
        knex.schema.alterTable('metadata', (table) => {
          table.string('key').notNullable().alter();
          table.string('value').notNullable().alter();
        }),
      )
      .then(() =>
        knex.schema.alterTable('tag', (table) => {
          table.string('key').notNullable().alter();
          table.string('value').notNullable().alter();
        }),
      )
      .then(() =>
        knex.schema.alterTable('version', (table) => {
          table
            .string('mimeType')
            .notNullable()
            .defaultTo('application/octet-stream')
            .alter();
        }),
      )
      ...
      .then(() =>
        knex.schema.raw(`CREATE TRIGGER audit_bucket_trigger
    AFTER UPDATE OR DELETE ON bucket
    FOR EACH ROW EXECUTE PROCEDURE audit.if_modified_func();`),
      )
      .then(() =>
        knex.schema.raw(`CREATE TRIGGER audit_bucket_permission_trigger
...
exports.down = function (knex) {
  return (
    Promise.resolve()
      ...
      .then(() =>
        knex.schema.raw(
          'DROP TRIGGER IF EXISTS audit_bucket_permission_trigger ON object_permission',
        ),
      )
      .then(() =>
        knex.schema.raw(
          'DROP TRIGGER IF EXISTS audit_bucket_trigger ON object',
        ),
      )
