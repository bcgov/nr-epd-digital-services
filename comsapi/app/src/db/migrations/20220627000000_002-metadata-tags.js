...
exports.up = function (knex) {
  return (
    Promise.resolve()
      ...
      .then(() =>
        knex.schema.createTable('version_metadata', (table) => {
      ...
      .then(() =>
        knex.schema.createTable('version_tag', (table) => {
      ...
      .then(() =>
        knex.schema.raw(`CREATE TRIGGER audit_version_metadata_trigger
    AFTER UPDATE OR DELETE ON version_metadata
    FOR EACH ROW EXECUTE PROCEDURE audit.if_modified_func();`),
      )
      // Create tag audit trigger
      .then(() =>
        knex.schema.raw(`CREATE TRIGGER audit_version_tag_trigger
...
exports.down = function (knex) {
  return (
    Promise.resolve()
      ...
      .then(() =>
        knex.schema.raw(
          'DROP TRIGGER IF EXISTS audit_version_tag_trigger ON version_tag',
        ),
      )
      .then(() =>
        knex.schema.raw(
          'DROP TRIGGER IF EXISTS audit_version_metadata_trigger ON version_metadata',
        ),
      )
