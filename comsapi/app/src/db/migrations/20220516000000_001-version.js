...
exports.up = function (knex) {
  return (
    Promise.resolve()
      ...
      .then(() =>
        knex.schema.raw(`CREATE TRIGGER audit_version_trigger
    ...
    Promise.resolve()
      ...
      .then((rows) => {
        ...
        if (data && data.length) {
          return Promise.all(
            data.map((row) => {
              return knex('object').where({ id: row.id }).update({
                originalName: row.originalName,
                mimeType: row.mimeType,
                createdAt: row.createdAt,
                updatedAt: row.updatedAt,
      ...
      .then(() =>
        knex.schema.raw(
          'DROP TRIGGER IF EXISTS audit_version_trigger ON version',
        ),
      )
