...
const controller = {
      ...
      await utils.trxWrapper(async (trx) => {
        ...
        const version = s3Response.VersionId
          ? await versionService.copy(
              ...
              objId,
              s3Response.CopyObjectResult?.ETag,
              userId,
              trx,
            )
        ...
        await metadataService.associateMetadata(
          version.id,
          getKeyValue(data.metadata),
          userId,
          trx,
        );

        // add tags to new version in DB
        await tagService.associateTags(
          version.id,
          getKeyValue(data.tags),
          userId,
          trx,
        );
      });
  ...
  async addTags(req, res, next) {
    try {
      ...
      const newTags = Object.entries({ ...req.query.tagset }).map(([k, v]) => ({
        Key: k,
        Value: v,
      }));
      ...
      const newSet = newTags
        ...
        .filter(
          (element, idx, arr) =>
            arr.findIndex((element2) => element2.Key === element.Key) === idx,
        )
        // add 'coms-id' tag
  ...
  async createObject(req, res, next) {
    try {
      ...
      const bucketId = req.query.bucketId
        ? addDashesToUuid(req.query.bucketId)
        : undefined;
      if (bucketId && userId) {
  ...
  async deleteObject(req, res, next) {
    try {
      ...
      res
        .status(200)
        .json(renameObjectProperty(s3Response, 'VersionId', 's3VersionId'));
    } catch (e) {
      next(errorToProblem(SERVICE, e));
    }
  },
  ...
  async deleteTags(req, res, next) {
    try {
      ...
      // Generate object subset by subtracting/omitting defined keys via filter/inclusion
      const keysToRemove = req.query.tagset
        ? Object.keys(req.query.tagset)
        : [];

      let newTags = [];
      if (keysToRemove.length && sourceObject.TagSet) {
        newTags = sourceObject.TagSet.filter(
          (x) => !keysToRemove.includes(x.Key) && x.Key != 'coms-id',
        );
      }
      ...
      await utils.trxWrapper(async (trx) => {
        ...
        } else if (sourceObject.TagSet && sourceObject.TagSet.length) {
          // remove all existing except coms-id
          dissociateTags = toLowerKeys(sourceObject.TagSet).filter(
            (x) => x.key != 'coms-id',
          );
        }

        if (dissociateTags.length)
          await tagService.dissociateTags(version.id, dissociateTags, trx);
      });
  ...
  async fetchMetadata(req, res, next) {
    try {
      ...
      if (config.has('server.privacyMask')) {
        params.userId = await userService.getCurrentUserId(
          getCurrentIdentity(req.currentUser, SYSTEM_USER),
        );
      }
    ...
    try {
      ...
      try {
        ...
        if (err instanceof Problem)
          throw err; // Rethrow Problem type errors
      ...
      const dbResponse = await utils.trxWrapper(async (trx) => {
        ...
        if (s3Response.VersionId) {
          // Create new version if bucket versioning enabled
          ...
          // Update existing version when bucket versioning not enabled
