...
import { createBucket, createObject, deleteBucket, deleteObject } from './services/coms.service';
...
const Invoice: React.FC = () => {
  ...
  const [attachmentsToDelete, setAttachmentsToDelete] = useState<DeletedAttachment[]>([]);
    ...
    if(!!id) {
      if (invoiceData?.getInvoiceById?.data) {
        if (invoiceData?.getInvoiceById?.data?.invoiceAttachments?.length > 0) {
          const firstAttachment = invoiceData?.getInvoiceById?.data?.invoiceAttachments[0];
    ...
    try {
      ...
      const updatedAttachments = await Promise.all(
        invoice?.invoiceAttachments?.map(async (attachment: any) => {
          if (!attachment?.file || (attachment?.objectId && !attachment?.previewUrl)){
          ...
          const comsResponse = await createObject(currentBucketId, attachment?.file);
  ...
  const updateInvoiceDetails = (invoice: any) => {
    const cleanInvoice = cleanGraphQLPayload(invoice, ['recipient', 'whoUpdated', '__typename']);
    const updatedInvoiceItems: UpdateInvoiceItem[] = cleanInvoice?.invoiceItems?.map((item: any) => {
    ...
    switch (action) {
      ...
      case InvoiceActions.SAVE_INVOICE:
        ...
        if (errors?.length > 0) {
          ...
          try {
            if(!!id) {
                const updatedInvoice = await processInvoiceAttachments(invoiceDetails);
                if (hasErrors) return;
              ...
              const { invoiceAttachments, ...restInvoice} = cleanGraphQLPayload(invoiceDetails, ['recipient', '__typename']);
              await createInvoice({ 
                variables: { 
                  invoice: {
                    ...restInvoice, 
              ...
              .then(async (response: any) => {
                if(response?.data?.createInvoice?.success && !!response?.data?.createInvoice?.data?.id) {
  ...
  const attachmentsChangeHandler = (event: any) => {
    ...
    if(property.includes('remove'))
    {
      setInvoiceDetails((prev: any) => {
        ...
        // Remove from UI
        const updated = prev.invoiceAttachments.filter((item: any) => item.id !== row.id);
      ...
      <PageContainer customContainerClass={'invoice_page_container'} role="invoice">
        ...
        { 
          <>
           ...
           { viewMode === UserMode.EditMode &&
              <div className='d-flex flex-column gap-1'>
                <label className='custom-invoice-edit-lbl'>Attach File(S)</label>
                <FileUploader multiple={true}maxSizeMB={10} 
                  onValidationError={(msg) => { setErrors(prev => [
                                        ...prev,
                                        {
                                          errorMessage: msg,
                                        },
                  ...
                  classNames={
                    {
                      ...
                      iconWrapper: 'd-flex align-items-center justify-content-center gap-2 custom-fu-icon-wrapper',
                      label:'custom-invoice-txt p-0 m-0'
                    }
                  }
                  customText={
                    { 
                      dropMessage: 'Drag file here or click to select from browser' 

                    }
                  }
                  onFileSelect={({ files, previewUrls }) => handleFileSelect(files, previewUrls)}
                />
              </div>
            }
            {/* Attachments */}
        ...
        }
        {
          ...
          >
            <Form
              ...
              formData={{recordPaymentDate: new Date(), recordPaymentAmount: 0,}}
