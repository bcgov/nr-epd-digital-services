...
const FORM_API: string =
    import.meta.env.VITE_FORM_API ||
        ((window as any)._env_ && (window as any)._env_.VITE_FORM_API);
const FORM_BACKEND_API: string =
    import.meta.env.VITE_FORM_BACKEND_API ||
        ((window as any)._env_ && (window as any)._env_.VITE_FORM_BACKEND_API);
const FORM_FLOW_API: string =
    import.meta.env.VITE_FORM_FLOW_API ||
        ((window as any)._env_ && (window as any)._env_.VITE_FORM_FLOW_API);
...
export const getFormDetails = async (formId?: string) => {
    try {
      ...
      const response = await getAxiosInstance(FORM_API).get(
        GET_FORM_ENDPOINT,
      );
 ...
 */
export const getApplicationFormData = async (formId: string, submissionId: string) => {
    try {
