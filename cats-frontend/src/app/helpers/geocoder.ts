...
export const apiCall = async <T>( url: string, method: Method, data?: object, headers?: object): Promise<T> => {
  try 
  {
    const response: AxiosResponse<T> = await axiosInstance({ url, method, data, headers: headers || {}});
