    ...
    httpPOSTRequest(URL, data, UserService.getToken())
      .then((res) => {
        if (res.data) {
          ...
          toast.success(
            <Translation>{(t) => t("Upload successfull")}</Translation>,
          );
      ...
      .catch((error) => {
        this.setState({ loading: false });
        toast.error(
          <Translation>{(t) => t("Failed to upload file")}</Translation>,
        );
        done(error);
      });
  };
  ...
  fetchCMISfile = (e, ...rest) => {
    ...
    httpGETRequest(downloadURL, {}, UserService.getToken())
      .then((res) => {
        ...
        this.downloadFIle();
        toast.success(
          <Translation>{(t) => t("Download successfull")}</Translation>,
        );
      })
      .catch((error) => {
        done(error);
        this.setState({ loading: false });
        toast.error(
          <Translation>{(t) => t("Failed to download file")}</Translation>,
        );
      });
  };
    ...
    if (this.state.selectedFile) {
      return (
        <div>
          ...
          {this.state.fileName && (
            <h6 style={{ color: "#ed6c6c", paddingTop: "3px" }}>
        ...
        {this.state.uploadSuccess && (
          <span style={{ fontWeight: "bold" }}>
            <i
              ...
              style={{ fontSize: "22px", color: "red", paddingRight: "6px" }}
            ></i>
            {Name}
          </span>
        )}
        ...
        {disabled && Name
          ? Name && (
              <div>
                ...
                {img && !this.state.loading
                  ? img && (
                      <div id="downloadImage" style={{ marginTop: "10px" }}>
          ...
          : disabled && <input type="file" disabled />}
      </div>
    );
  }
}
