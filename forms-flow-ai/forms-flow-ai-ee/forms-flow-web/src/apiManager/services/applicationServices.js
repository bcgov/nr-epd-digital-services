...
export const FilterApplications = (params, ...rest) => {
  const done = rest.length ? rest[0] : () => {};
  return (dispatch) => {
    const { applicationName, id, modified, applicationStatus, created } =
      params.filters || {};
    ...
    if (created && created?.filterVal?.length === 2) {
      let createdFrom = moment
        .utc(created.filterVal[0])
        .format("YYYY-MM-DDTHH:mm:ssZ")
        .replace(/\+/g, "%2B");
      let createdTo = moment
        .utc(created.filterVal[1])
        .format("YYYY-MM-DDTHH:mm:ssZ")
        .replace(/\+/g, "%2B");
      url += `&createdFrom=${createdFrom}&createdTo=${createdTo}`;
    }
