...
const View = React.memo((props) => {
  ...
  const bundleSubmission = useSelector(
    (state) => state.bundle.bundleSubmission?.data
  );
  ...
  useEffect(() => {
    if (processData.formType === TYPE_BUNDLE) {
      setDraftData(bundleSubmission);
      draftRef.current = bundleSubmission;
    }
  }, [bundleSubmission, processData]);
      ...
      >
        <div className="ml-4 mr-4">
          <Confirm
            ...
            message={`${t("Are you sure you wish to delete the draft")} "${textTruncate(14, 12, draftDelete.draftName)
              }" 
          ...
          />
          {processData?.status === "active" ? (
            processData.formType === TYPE_BUNDLE ? (
              <BundleSubmit
                onSubmit={(data) => {
                  setPoll(false);
                  exitType.current = "SUBMIT";
...
const doProcessActions = (submission, ownProps) => {
  return (dispatch, getState) => {
    ...
    const loadingDispatch = processData.formType === TYPE_BUNDLE
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    onSubmit: (submission, formId, isPublic, processData) => {
      const isBundle = processData.formType === TYPE_BUNDLE;
