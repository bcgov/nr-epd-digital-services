...
class FormResourceRenderPdf(Resource):
    ...
    def get(form_id: string, submission_id: string):
        ...
        is_bundle = bool(request.args.get("is_bundle"))
        mapper_id = request.args.get("mapper_id", None)
...
class FormResourceExportPdf(Resource):
    ...
    @profiletime
    def post(form_id: string, submission_id: string):  # pylint:disable=too-many-locals
        ...
        is_bundle = bool(request.args.get("isBundle"))
        mapper_id = request.args.get("mapperId", None)
        if is_bundle and not mapper_id:
            raise BusinessException(
                "Form Mapper ID Not Found !", HTTPStatus.BAD_REQUEST
            )
        pdf_service = PDFService(
            form_id=form_id,
            submission_id=submission_id,
